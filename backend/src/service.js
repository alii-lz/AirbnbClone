...
export const register = (email, password, name) =>
  resourceLock((resolve, reject) => {
    if (!email) {
      return reject(
        new InputError('Must provide an email for user registration')
      );
    } else if (!password) {
      return reject(
        new InputError('Must provide a password for user registration')
      );
    } else if (!name) {
      return reject(
        new InputError('Must provide a name for user registration')
      );
    } else if (email && email in users) {
...
export const addListing = (title, email, address, price, thumbnail, metadata) =>
  resourceLock((resolve, reject) => {
    if (title === undefined) {
      ...
      Object.keys(listings).find((key) => listings[key].title === title) !==
      undefined
    ) {
      ...
      return reject(
        new InputError('Must provide a valid price for new listing')
      );
    } else if (thumbnail === undefined) {
      return reject(new InputError('Must provide a thumbnail for new listing'));
    } else if (metadata === undefined) {
      return reject(
        new InputError('Must provide property details for this listing')
      );
    } else {
      const id = newListingId();
      listings[id] = newListingPayload(
        title,
        email,
        address,
        price,
...
export const leaveListingReview = (email, listingId, bookingId, review) =>
  resourceLock((resolve, reject) => {
    if (!(bookingId in bookings)) {
      ...
      return reject(
        new InputError('This booking is not associated with this listing ID')
      );
...
export const makeNewBooking = (owner, dateRange, totalPrice, listingId) =>
  resourceLock((resolve, reject) => {
    if (!(listingId in listings)) {
      ...
      return reject(
        new InputError('Must provide a valid date range for the booking')
      );
    } else if (
      ...
      return reject(
        new InputError('Must provide a valid total price for this booking')
      );
    } else if (listings[listingId].owner === owner) {
      return reject(
        new InputError('Cannot make bookings for your own listings')
      );
    } else if (listings[listingId].published === false) {
      return reject(
        new InputError('Cannot make a booking for an unpublished listing')
      );
    } else {
...
export const acceptBooking = (owner, bookingId) =>
  resourceLock((resolve, reject) => {
    if (!(bookingId in bookings)) {
      ...
      Object.keys(listings).find(
        (key) =>
          key === bookings[bookingId].listingId && listings[key].owner === owner
      ) === undefined
    ) {
      return reject(
        new InputError("Cannot accept bookings for a listing that isn't yours")
      );
